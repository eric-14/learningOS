...
!call BIOS INIT3 TO LOAD BSECTORS to (segment,offset) = (0x9800,0)
    ...
    xor cx,cx !ch=cyl=0 -- track, cl=sector=0
    incb cl !BIOS  counts sectors from 1 
    xor bx, bx ! (ES, BX) = real address = (0x9800,0)
    movb ah, #2 ! ah=READ 
    movb al, #BSECTORS ! al= number of sectors to load

    int 0x13 !call bios input and output function
...
next:
    mov ax, cs !set CPU segment registers to 0x9800 CS gives current address of the current position
    mov ds,ax  ! we know ES=CS=0x9800 , let DS=CS
    mov ss, ax  ! let SS  = CS 
    mov sp , #SSP !SSP is higher address to give room for booter
    call _main 

    !OSSEG is just after the ROM Area
    jmpi 0, OSSEG ! 0 is the value of the IP and OSSEG the value of CS --START OF THE EXECUTION OF THE BOOTER

! Input and output functions 

_getc: !char getc():return and input char
    xorb ah, ah
